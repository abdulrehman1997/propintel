...
describe("idempotent upserts", () => {
  ...
  it("upsertRentBenchmark is keyed on (zip, bedroom, source, period)", async () => {
    await upsertRentBenchmark(client, {
      zip: "17101",
      bedroom: 2,
      fmr: 1100,
      source: "HUD_FMR",
    ...
    const { rows } = await client.query(
      "SELECT fmr FROM rent_benchmarks WHERE zip=$1 AND bedroom=2",
      ["17101"],
    );
    expect(rows).toHaveLength(1);
  ...
  it("upsertEconomicIndicator is keyed on (geo, metric, source, period)", async () => {
    await upsertEconomicIndicator(client, {
      geo: "17101",
    ...
    const { rows } = await client.query(
      "SELECT value FROM economic_indicators WHERE geo=$1 AND metric='median_income'",
      ["17101"],
    );
    expect(rows).toHaveLength(1);
  ...
  it("startRefresh + finishRefresh write a refresh_log row", async () => {
    ...
    const { rows } = await client.query(
      "SELECT * FROM refresh_log WHERE id=$1",
      [id],
    );
    expect(rows).toHaveLength(1);

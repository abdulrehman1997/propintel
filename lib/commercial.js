...
export const analyzeCommercial = (input) => {
  ...
  const gpr = units.reduce(
    (s, u) => s + (u.count || 1) * (u.marketRent || 0) * 12,
    0,
  );
  const gsr = units.reduce(
    (s, u) => s + (u.count || 1) * (u.inPlaceRent || 0) * 12,
    0,
  );
...
export const projectCommercial = (input) => {
  ...
  for (let y = 1; y <= holdYears; y++) {
    ...
    const remainingBal = loanBalance(
      debt.maxLoan,
      frac(interestRate),
      amortYears,
      y,
    ...
    if (y < holdYears) {
      ...
      const saleProceeds = netSaleProceeds(
        saleValue,
        frac(saleCostPct),
        remainingBal,
      );

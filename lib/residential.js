...
export const projectResidential = (input) => {
  ...
  const resolvedExitCapRate = input.exitCapRate != null
  ...
  const saleValue = resolvedExitCapRate != null && resolvedExitCapRate > 0
  ...
  const saleProceeds = Math.max(0, netSaleProceeds(saleValue, saleCost, exitYear.remainingBalance));

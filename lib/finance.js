...
export const sizeMaxLoan = ({ noi, value, maxLTV, minDebtYield, minDSCR, rate, amortYears, interestOnly = false, ltvBase }) => {
  const K = mortgageConstant(rate, amortYears, { interestOnly });
  // The Max-LTV constraint must be measured against the amount actually invested
  // (the purchase price), NOT the income-implied value (NOI / going-in cap). When
  // the assumed cap rate makes the implied value diverge from the price, an
  // income-based LTV cap can size loans above the real purchase price (>100% LTV).
  // Callers pass `ltvBase` (purchase price); fall back to `value` for back-compat.
  const ltvValue = ltvBase != null ? ltvBase : value;
  // When NOI is non-positive the deal cannot service any debt. Income-based
  // constraints (debt yield, DSCR) would otherwise go negative and the LTV
  // candidate would win, producing a meaningless negative loan. Floor at 0.
  if (noi <= 0) {
    return {
      loanLTV: maxLTV * ltvValue,
      loanDebtYield: 0,
      loanDSCR: 0,
      maxLoan: 0,
      bindingConstraint: 'NOI',
      mortgageConstant: K,
    };
  }
  const loanLTV = maxLTV * ltvValue;
...
export const equityMultiple = (distributions, equityInvested) => {
  ...
  // Equity multiple = total distributions / equity invested. A negative result is
  // mathematically impossible (you cannot get back less than nothing), so floor at 0.
  const total = distributions.reduce((a, b) => a + b, 0);
  return Math.max(0, total / equityInvested);
};

...
describe("CommercialInputs", () => {
  it("renders rentable square feet and the rent roll", () => {
    render(
      <CommercialInputs inputs={inputs} onChange={() => {}} errors={{}} />,
    );
  ...
  it("calls onChange with rentableSqft on edit", async () => {
    ...
    render(
      <CommercialInputs inputs={inputs} onChange={onChange} errors={{}} />,
    );
  ...
  it("shows a commercial validation error", () => {
    ...
    expect(
      screen.getByText("Square feet must be greater than 0"),
    ).toBeInTheDocument();
  });
});

...
const inputCls =
  "w-full px-4 py-2.5 bg-paper-50 border border-paper-200 rounded-xl focus:ring-2 focus:ring-forest-300 focus:border-forest-400 outline-none transition-colors duration-200 tabular-nums text-sm text-ink-900";
...
export const CommercialInputs = ({ inputs, onChange, errors = {} }) => {
  ...
  const addUnit = () =>
    onChange("units", [
      ...units,
      { count: 1, marketRent: 1500, inPlaceRent: 1450 },
    ]);
  const removeUnit = (idx) =>
    onChange(
      "units",
      units.filter((_, i) => i !== idx),
    );
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <InputGroup label="Rentable Square Feet">
            <input
              ...
              onChange={(e) => {
                handle("squareFeet", e.target.value);
                handle("rentableSqft", e.target.value);
              }}
      ...
      <div>
        <SectionTitle icon={DollarSign}>
          {isMultifamily ? "Rent Roll (per unit type)" : "Income"}
        </SectionTitle>
        {isMultifamily ? (
          <div className="space-y-3">
            {units.map((u, idx) => (
              <div
                key={idx}
                className="grid grid-cols-3 gap-2 items-end bg-slate-50 rounded-lg p-2"
              >
                <InputGroup label="Count">
                  <input
                    type="number"
                    value={u.count}
                    onChange={(e) => updateUnit(idx, "count", e.target.value)}
                    className={inputCls}
                  />
                </InputGroup>
                <InputGroup label="Market Rent">
                  <input
                    ...
                    value={u.marketRent}
                    onChange={(e) =>
                      updateUnit(idx, "marketRent", e.target.value)
                    }
                    className={inputCls}
                  />
                </InputGroup>
                <InputGroup label="In-Place Rent">
                  <div className="flex gap-1 items-center">
                    <input
                      ...
                      value={u.inPlaceRent}
                      onChange={(e) =>
                        updateUnit(idx, "inPlaceRent", e.target.value)
                      }
                      className={inputCls}
                    />
                    <button
                      ...
                      className="text-slate-400 hover:text-rose-500 text-lg px-1"
                    >
                      ×
                    </button>
                  </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            <InputGroup
              label="Total Annual Market Rent ($/yr)"
              tooltip="Entered as a single unit row at 1 SF-equivalent."
            >
              <Money
        ...
        <div className="grid grid-cols-2 gap-4 mt-2">
          <InputGroup label="Lease Type">
            <select
              value={inputs.leaseType || "gross"}
              onChange={(e) => handle("leaseType", e.target.value)}
              className={inputCls}
            >
          ...
          <InputGroup
            label={`Recovery Ratio: ${Math.round((inputs.recoveryRatio ?? 0) * 100)}%`}
            tooltip="Share of OpEx reimbursed by tenants (NNN/MG)."
          >
            <input
              type="range"
              min="0"
              max="1"
              step="0.05"
              value={inputs.recoveryRatio ?? 0}
              onChange={(e) => handle("recoveryRatio", e.target.value)}
              className="w-full"
            />
          </InputGroup>
          <div className="col-span-2">
            <InputGroup label="Annual Operating Expenses">
              <Money
                value={inputs.opexAnnual ?? inputs.annualOperatingExpenses ?? 0}
                onChange={(e) => {
                  handle("opexAnnual", e.target.value);
                  handle("annualOperatingExpenses", e.target.value);
                }}
              />
            </InputGroup>
          </div>
          ...
          <InputGroup label={`Credit Loss: ${inputs.creditLossPct ?? 0}%`}>
            <input
              ...
              max="10"
              step="0.5"
              value={inputs.creditLossPct ?? 0}
              onChange={(e) => handle("creditLossPct", e.target.value)}
              className="w-full"
            />
          </InputGroup>
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <InputGroup label="Max LTV %">
            <Pct
              value={inputs.maxLTV ?? 75}
              onChange={(e) => handle("maxLTV", e.target.value)}
              step="1"
            />
          </InputGroup>
          <InputGroup label="Min DSCR">
            <input
              type="number"
              step="0.05"
              value={inputs.minDSCR ?? 1.25}
              onChange={(e) => handle("minDSCR", e.target.value)}
              className={inputCls}
            />
          </InputGroup>
          <InputGroup label="Min Debt Yield %">
            <Pct
              value={inputs.minDebtYield ?? 8}
              onChange={(e) => handle("minDebtYield", e.target.value)}
              step="0.5"
            />
          </InputGroup>
          ...
          <InputGroup label="Amortization (Years)">
            <input
              type="number"
              value={inputs.amortYears ?? inputs.loanTermYears ?? 30}
              onChange={(e) => {
                handle("amortYears", e.target.value);
                handle("loanTermYears", e.target.value);
              }}
              className={inputCls}
            />
          </InputGroup>
          <InputGroup label="Interest-Only">
            <select
              value={inputs.interestOnly ? "yes" : "no"}
              onChange={(e) => handle("interestOnly", e.target.value === "yes")}
              className={inputCls}
            >
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <InputGroup label="Sale Cost %">
            <Pct
              value={inputs.saleCostPct ?? 2}
              onChange={(e) => handle("saleCostPct", e.target.value)}
              step="0.5"
            />
          </InputGroup>
          <InputGroup label="Rent Growth %">
            <Pct
          ...
          <InputGroup label="Expense Growth %">
            <Pct

...
const fmtX = (v) =>
  v == null || Number.isNaN(v) ? "N/A" : `${Number(v).toFixed(2)}x`;
const fmtNum = (v, d = 2) =>
  v == null || Number.isNaN(v) ? "N/A" : Number(v).toFixed(d);

const RedFlags = ({ flags }) => {
  if (!flags || flags.length === 0) return null;
  return (
    <div className="px-8 pb-6 space-y-1.5">
      <h4 className="text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-2">
        Red-Flag Gates
      </h4>
      {flags.map((f) => (
        <div
          key={f.code}
          className={cn(
            "text-xs font-medium border rounded-lg px-3 py-2 flex items-center gap-2",
            SEVERITY_STYLE[f.severity] ?? SEVERITY_STYLE.MEDIUM,
          )}
        >
...
const ResidentialMetrics = ({ r }) => (
  <div className="p-8 grid grid-cols-2 md:grid-cols-3 gap-4">
    <MetricCard
      ...
      benchmark={{ green: 200, red: 0 }}
    />
    <MetricCard
      label="CoC Return"
      value={formatPercent(r.cashOnCash)}
      benchmark={{ green: 6, red: 0 }}
    />
    <MetricCard
      label="Cap Rate"
      value={formatPercent(r.capRate)}
      benchmark={{ green: 5, red: 3 }}
    />
    <MetricCard
      label="IRR"
      value={formatPercent(r.irr)}
      benchmark={{ green: 12, red: 0 }}
      tooltip="Levered internal rate of return over the hold period."
    />
    <MetricCard
      label="Equity Multiple"
      value={fmtX(r.equityMultiple)}
      benchmark={{ green: 1.8, red: 1 }}
      tooltip="Total distributions ÷ equity invested."
    />
    <MetricCard
      label="DSCR"
      value={fmtNum(r.dscr)}
      benchmark={{ green: 1.25, red: 1 }}
      tooltip="NOI ÷ annual debt service."
    />
    <MetricCard
      label="Debt Yield"
      value={formatPercent(r.debtYield)}
      benchmark={{ green: 10, red: 8 }}
      tooltip="NOI ÷ loan amount."
    />
    <MetricCard
      label="GRM"
      value={r.GRM != null ? r.GRM.toFixed(1) : "N/A"}
      benchmark={{ green: -12, red: -20 }}
    />
    <MetricCard
      label="1% Rule"
      value={r.onePercentRule >= 1 ? "✅ PASS" : "❌ FAIL"}
      benchmark={{ green: 1.0, red: 0.7 }}
    />
    <MetricCard
      label="Exit Value"
      value={formatCurrency(r.exitValue)}
      tooltip="Projected terminal sale value at end of hold."
    />
    {r.exitCapRate != null && (
    ...
    <MetricCard
      label="Annual ROI"
      value={formatPercent(r.annualROI)}
      benchmark={{ green: 8, red: 2 }}
    />
  </div>
);
...
const CommercialMetrics = ({ r }) => {
  const perKey =
    ASSET_LABELS[r.assetType] === "units" ? "pricePerUnit" : "pricePerSF";
  const perLabel =
    ASSET_LABELS[r.assetType] === "units" ? "Price / Unit" : "Price / SF";
  return (
    <>
      <div className="p-8 pb-0 grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <MetricCard
          label="Going-In Cap"
          value={formatPercent(r.goingInCap)}
          benchmark={{ green: 6, red: 4 }}
        />
        <MetricCard label="Exit Cap" value={formatPercent(r.exitCapRate)} />
        <MetricCard
          label="OER"
          value={formatPercent(r.oer)}
          tooltip="Operating expense ratio (effective opex ÷ EGI)."
        />
        <MetricCard label={perLabel} value={formatCurrency(r[perKey])} />
        <MetricCard
          label="Implied Value"
          value={formatCurrency(r.value)}
          tooltip="NOI ÷ going-in cap rate."
        />
      </div>
      <div className="px-8 pt-4 pb-0">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <MetricCard
            ...
            value={formatCurrency(r.equity)}
          />
          <MetricCard
            label="DSCR"
            value={fmtNum(r.dscr)}
            benchmark={{ green: 1.3, red: 1 }}
          />
          <MetricCard
            label="Debt Yield"
            value={formatPercent(r.debtYield)}
          ...
          <MetricCard
            ...
            value={formatCurrency(r.annualDebtService)}
          />
        </div>
      </div>
      <div className="p-8 grid grid-cols-2 md:grid-cols-3 gap-4">
        <MetricCard
          label="Levered IRR"
          value={formatPercent(r.leveredIRR)}
          benchmark={{ green: 14, red: 0 }}
        />
        <MetricCard
          label="Unlevered IRR"
          value={formatPercent(r.unleveredIRR)}
        ...
        <MetricCard
          ...
          value={formatPercent(r.breakEvenOccupancy)}
          benchmark={{ green: -85, red: -95 }}
        />
      </div>
    </>
  );
      ...
      <div className="px-8 py-10 text-center border-b border-paper-200 bg-paper-100/50">
        <div className="flex flex-col items-center">
          ...
          <div className="font-display text-2xl font-medium text-ink-900 mb-1.5 flex items-center gap-2">
            ...
            {blended && (
              <Tooltip text="Score includes neighborhood context factors (income, vacancy, employment)." />
            )}
          </div>

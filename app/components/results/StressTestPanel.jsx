...
export const StressTestPanel = ({ scenarios }) => {
  if (!scenarios || scenarios.length === 0) {
    return <p className="text-xs text-slate-400 italic py-4">Stress tests run on residential deals.</p>;
  }
  return (
        ...
        <tbody className="divide-y divide-slate-50">
          {scenarios.map((s) => (
            <tr key={s.scenario} data-scenario={s.scenario}>
              <td className="py-2 font-medium text-slate-700">{LABELS[s.scenario] ?? s.scenario}</td>
              <td className="py-2 text-right font-mono text-slate-600">{formatCurrency(s.noi)}</td>
              <td className={cn('py-2 text-right font-mono', s.annualCashFlow > 0 ? 'text-emerald-600' : 'text-rose-600')}>
                {formatCurrency(s.annualCashFlow)}
              </td>
              <td className="py-2 text-right font-mono text-slate-600">{s.dscr.toFixed(2)}</td>
              <td className="py-2 text-center"><Pass ok={s.dscrPass} /></td>
              <td className="py-2 text-center"><Pass ok={s.cashFlowPass} /></td>
            </tr>
          ))}
        </tbody>
      </table>

...
describe('residentialInputSchema', () => {
  ...
  it('rejects vacancy > 100', () => {
    const r = validateInput(residentialInputSchema, { ...valid, vacancyPct: 150 });
    expect(r.ok).toBe(false);
  });
});
...
describe('commercialInputSchema', () => {
  it('accepts a valid commercial input', () => {
    const r = validateInput(commercialInputSchema, {
      ...
      vacancyPct: 5, opexAnnual: 400000,
      goingInCapRate: 8, exitCapRate: 8.5,
      maxLTV: 70, minDSCR: 1.25, minDebtYield: 8,
      interestRate: 6.5, amortYears: 25,
      holdYears: 5, rentGrowthPct: 3, expenseGrowthPct: 3,
    });
    expect(r.ok).toBe(true);
  });

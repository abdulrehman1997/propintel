...
describe('projectResidential exit cap rate', () => {
  ...
  const inputWithCap = {
    purchasePrice: 250000, repairCosts: 0, downPaymentPct: 20, interestRate: 6.5,
    loanTermYears: 30, annualPropertyTax: 3000, annualInsurance: 1200, monthlyHOA: 0,
    monthlyRent: 2500, vacancyPct: 5, managementPct: 8, maintenancePct: 1, capExPct: 5,
    rentGrowthPct: 3, expenseGrowthPct: 3, appreciationPct: 3, holdYears: 5,
  ...
  it('exit-cap path: terminal value differs from appreciation-only path', () => {
    ...
    expect(Math.abs(pCap.cashFlows[5] - pNoCap.cashFlows[5])).toBeGreaterThan(1000);
  });
  ...
  it('saleCostPct is threaded through from input (7% costs more than 6%)', () => {
    ...
    const p6 = projectResidential({ ...inputWithCap, saleCostPct: 6, exitCapRate: undefined });
    const p7 = projectResidential({ ...inputWithCap, saleCostPct: 7, exitCapRate: undefined });
